...
- `index.html` — the static UI shell (redesign "1a"): a 1560px two-column card — left column is the form (header row, seller identity chip, Buyer/Items/Invoice-info tabs, sticky action bar), right column is the 500px live-PDF-preview pane — plus the modals (stats, history, help, backup, onboarding, embed, past-invoice, YoY). Every interactive element has an id used by `main.js`. Translatable strings carry `data-i18n="<key>"`. The seller `s_*` inputs live inside the seller-chip dropdown's edit form and must stay in the DOM (the pipeline reads them by id).
- `src/main.js` (~7000 lines) — application logic. Organized as banner-comment sections (`// -------- <name> --------`). Major sections, in order: State / Helpers / i18n / Storage / Seller / Buyer / Invoice number / Filename pattern / Text-block presets / History / YoY / Statistics / Items / PDF drop / Country normalization / XML generation / Actions / Font loader / PDF dispatcher / PDF/A output intent / XML embedding / Backup / Help modal / History modal / Embed-XML modal / Redesign shell (fonts, tabs, menus, seller chip, onboarding, due chips, backup modal) / Init / Bootstrap. Use grep for the banner comment to jump to a section.
...
3. `makeDrawKit(pdfDoc, fonts, opts)` is **exported from `main.js` and imported by `layouts.js`** (circular import — intentional). It returns the per-page drawing API (`drawText`, `drawTextRight`, `drawTextCenter`, `drawRule`, `wrapText`, `widthAt`, plus `newPage()`, a `page` getter and constants) that all three renderers rely on. Renderers must call `kit = makeDrawKit(...)` per invoice; `newPage()` swaps the internal page that all draw helpers target — that's how renderers add pages.
4. `embedFacturXIntoPDF(pdfDoc, xml)` attaches the XML as `factur-x.xml` with `AFRelationship.Alternative`, sets PDF metadata, then calls `addPDFAOutputIntent` (sRGB ICC) and `injectFacturXMP` (PDF/A-3 + Factur-X XMP). `setPDFTrailerID` sets a random `/ID` in the trailer (PDF/A requires one). These steps together are what makes the PDF pass verapdf — don't skip any of them.
...
The UI font (IBM Plex Mono) reuses the same embedded WOFF data: `injectUIFontFaces()` injects runtime `@font-face` rules from `FONT_DATA['ibm-plex-mono']` — no CDN font, keep it that way. Headings use `'IBM Plex Sans'` with a system-sans fallback (Plex Sans itself is not bundled).

### i18n

Three UI languages (`de`, `en`, `fr`) and three invoice languages (independent of UI). `t(key)` resolves UI strings; `tInvoice(key)` resolves PDF/XML strings using the invoice language. The dictionary is `I18N` near the top of `main.js`. Default boilerplate texts (intro, payment note, greeting, signature, footnote) are **stored per invoice language** (autosaved, debounced, on input — there is no explicit save button) — when adding a new default text, wire it into the per-language storage shape, not a single field. Independently of that, the intro/payment-note/footnote blocks have a named **preset system** (`erechnung:textpresets:v1`, one flat list per block, not per language); legacy `erechnung:footnotes:v1` entries are migrated into the footnote block on first load.
...
- The Help modal is a two-pane topic browser: `HELP_TOPICS` in `main.js` holds eight English-only markdown bodies rendered via `renderMarkdown()` (a tiny subset of Markdown, not a full parser). The "Keyboard shortcuts" topic must stay in sync with the global `keydown` bindings (Esc, ⌘/Ctrl+Enter, ⌘/Ctrl+D, 1/2/3, ?).
